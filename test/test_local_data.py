...
class TestLocalData(TestCase):
    def test_local_wind_data(self):
        '''Pull local nc data using get_nc_data
        '''
    ...
    def test_odd_timestamp(self):
        '''Test failing timestamp
        '''
        filename = os.path.join(os.path.dirname(__file__), "53252.nc")
        #wkt = "POINT(-103.128662109375 40.24179856487036)"
        attributes = ["power", "wind_direction", "wind_speed", "temperature",
        ...
        leap_day = False
        utc = False
        start = pandas.to_datetime(1191110401*10**9).tz_localize('utc')
        end = pandas.to_datetime(1196294400*10**9).tz_localize('utc')
        wind_data = get_nc_data_from_file(filename, start, end, attributes=attributes, leap_day=leap_day, utc=utc)
        '''
        #wind_data = get_nc_data_from_file(filename, start, end, attributes=attributes, leap_day=leap_day, utc=utc)
